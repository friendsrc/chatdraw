...
public class ChatActivity extends BaseActivity implements RecyclerViewClickListener,
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    adapter = new ChatRecyclerViewAdapter(myDataset, ChatActivity.this, this);
  ...
  private void selectImage() {
    ...
    builder.setItems(items, (dialogInterface, i) -> {
      if (items[i].equals("Camera")) {
        ...
        if (ContextCompat
            .checkSelfPermission(ChatActivity.this, Manifest.permission.CAMERA)
        ...
        if (ContextCompat.checkSelfPermission(
            ChatActivity.this, Manifest.permission.READ_EXTERNAL_STORAGE)
            == PackageManager.PERMISSION_DENIED) {
          ActivityCompat.requestPermissions(
              ChatActivity.this,
    ...
    if (resultCode == Activity.RESULT_OK) {
      if (requestCode == REQUEST_CAMERA) {
        ...
        if (uriString.startsWith("content://")) {
          Cursor cursor = null;
          try {
            cursor = getContentResolver().query(pdfUri, null, null,
                null, null);
      ...
      if (isActionSelected) {
        ...
        if (bmp != null) {
          ...
          uploadTask.addOnFailureListener(e -> {
            ...
            progressDialog.dismiss();
            Toast.makeText(ChatActivity.this, "Upload successful",
                Toast.LENGTH_LONG).show();
            imageRef.getDownloadUrl().addOnSuccessListener(uri -> {
              String url = uri.toString();
              ChatItem newChatItem =
                  addMessageToAdapter(userUid + "\tIMAGE\t" + url);
            ...
            progressDialog.setProgress(currentProgress);
              ...
              ChatActivity.this, "Failed to upload images", Toast.LENGTH_SHORT)
              .show());
          ...
          uploadTask.addOnFailureListener(e -> {
            ...
            fileReference.getDownloadUrl().addOnSuccessListener(uri -> {
              ...
              databaseRef.child(userID).child("imageUrl").setValue(url);
          ...
          final StorageReference storageReference = googleStorageRef
              .getReference().child(userID).child("Uploads").child(fileName);
          storageReference.putFile(pdfUri)
              .addOnSuccessListener(taskSnapshot -> {
                ...
                storageReference.getDownloadUrl()
                    .addOnSuccessListener(uri -> {
                      ...
                      ChatItem newChatItem = addMessageToAdapter(
                          userUid + "\tPDF\t" + pdfName + "\t" + uri);
                      sendMessage(newChatItem);
                    });
              ...
              .addOnFailureListener(e -> {
                progressDialog.dismiss();
                Toast.makeText(
                    ChatActivity.this, "Failed to upload files", Toast.LENGTH_SHORT)
                    .show();
              }).addOnProgressListener(taskSnapshot -> {
                int currentProgress = (int) (
                    100 * taskSnapshot.getBytesTransferred() / taskSnapshot.getTotalByteCount());
        ...
        Toast.makeText(this, "No file selected or camera picture not configured yet",
            Toast.LENGTH_SHORT).show();
      }
    }
  }
        ...
        if (connectivityManager.getNetworkInfo(ConnectivityManager.TYPE_MOBILE).getState()
          ...
          if (isGroup) {
            if (isServiceReady) {
              ...
              Toast.makeText(this, "Calling service not ready",
                  Toast.LENGTH_SHORT).show();
            }
          } else {
            if (isServiceReady) {
  ...
  private void groupCallButtonClicked() {
    ...
    if (userImageUrl[0] == null) {
      userImageUrl[0] = "https://firebasestorage.googleapis.com/v0/b/"
          + "chatdraw-ff7eb.appspot.com/o/Users%2F106689861101623002819%2Fprofilepic%"
          + "2F1567929062984.jpg?alt=media&token=a800b643-1d02-4d38-964f-46c84b0b3b02";
    }
    ...
    if (getSinchServiceInterface().getGroupIsOnGoingCall()) {
      if (getSinchServiceInterface().getGroupUserName().equals(sinchGroupID)) {
        ...
        Toast.makeText(
            this, "Cannot call others while talking with others", Toast.LENGTH_SHORT)
            .show();
      }
    } else {
      if (getSinchServiceInterface().getIsOnGoingCall()) {
        ...
        Toast.makeText(this, "Cannot call others while trying to connect with others",
            Toast.LENGTH_SHORT).show();
      } else {
        if (sinchGroupID.isEmpty()) {
          Toast.makeText(this, "Please enter a user to call", Toast.LENGTH_LONG)
              .show();
          return;
        }
        ...
        try {
          ...
          ActivityCompat.requestPermissions(
              this, new String[]{e.getRequiredPermission()}, REQUEST_MICROPHONE);
        }
      }
    }
  }
  ...
  private void callButtonClicked() {
    if (getSinchServiceInterface().getIsOnGoingCall()) {
      if (getSinchServiceInterface().getFriendUserName().equals(friendsUid)) {
        ...
        Toast.makeText(this, "Cannot call others while talking with others",
            Toast.LENGTH_SHORT).show();
      }
    } else {
      if (getSinchServiceInterface().getGroupIsOnGoingCall()) {
        ...
        if (!getSinchServiceInterface().getTryConnectUser().equals(friendsUid)) {
          ...
          Intent callScreen = new Intent(this, CallScreenActivity.class);
          callScreen
              .putExtra(SinchService.CALL_ID, getSinchServiceInterface().getTryConnectCallID());
          callScreen.putExtra("userID", userUid);
          callScreen.putExtra("FriendID", friendsUid);
          callScreen.putExtra("FriendName", friendName[0]);
          startActivity(callScreen);
        ...
        if (userNameTemp.isEmpty()) {
          Toast.makeText(this, "Please enter a user to call",
              Toast.LENGTH_LONG).show();
          return;
        }
        ...
        try {
          ...
          if (call == null) {
            ...
            Toast.makeText(this, "Service is not started. "
                    + "Try stopping the service and starting it again before placing a call.",
                Toast.LENGTH_LONG).show();
            return;
          }
  ...
  /**
   * Gets messages for this chatroom from firebase and populate the <code>RecyclerView</code>.
   */
      ...
      db.collection("GroupMessages")
          ...
          .addSnapshotListener((queryDocumentSnapshots, e) -> {
            ...
            for (DocumentSnapshot q : queryDocumentSnapshots) {
              ...
              if (chatItem != null && !chatItem.getSenderID().equals(userUid)) {
                String updateImageUrl = friendImageUrl[0];
                chatItem.setSenderImageUrl(updateImageUrl);
              }
              adapter.addData(chatItem);
              recyclerView.scrollToPosition(adapter.getItemCount() - 1);
            }
  ...
  /**
   * Creates a <code>ChatItem</code> from the given <code>messageBody</code> and add the
   * <code>ChatItem</code> to the RecyclerViewAdapter.
   * @param messageBody message body that would be added
   * @return <code>ChatItem</code> that is created
   */
  ...
  public void recyclerViewListClicked(View v, int position) {
    ChatItem chatItem = adapter.getItem(position);
  ...
  /**
   * Navigates to the activity that displays the image contained in the given <code>ChatItem</code>.
   * @param chatItem <code>ChatItem</code> that contains the image
   */
  ...
  /**
   * Gets messages preceeding the messages that is currently displayed.
   */
  ...
  /**
   * Shows a popup window displaying the image contained in the given <code>ChatItem</code>.
   * @param chatItem <code>ChatItem</code> containing the image
   */
  public void showPhotoPopup(ChatItem chatItem) {
    ...
    ImageView photo = dialog.findViewById(R.id.photo_popup_image);
    String[] arr = chatItem.getMessageBody().split("\t");
    ...
    ImageView closeButton = dialog.findViewById(R.id.photo_popup_close_button);
    closeButton.setOnClickListener(v -> dialog.dismiss());

    ImageView saveButton = dialog.findViewById(R.id.photo_popup_save_button);
    saveButton.setOnClickListener(v -> Toast.makeText(ChatActivity.this, "Not yet configured",
        Toast.LENGTH_SHORT).show());
  ...
  //              Toast.makeText(ChatActivity.this, "Unknown error occurred [802]",
  //                  Toast.LENGTH_SHORT).show();
  //              e.printStackTrace();
  //            }
  //          }
  ...
  //                num_participant = 0;
  //                Toast.makeText(ChatActivity.this, "No call before",
  //                    Toast.LENGTH_SHORT).show();
  //              }
  //            } catch (Exception e) {
  //              Toast.makeText(ChatActivity.this, "Unknown error occurred [804]",
  //                  Toast.LENGTH_SHORT).show();
  //            }
  //          }
  //        }
